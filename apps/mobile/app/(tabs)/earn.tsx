...
function EarnMethod({
  ...
  const bg = tone === "cyan" ? "rgba(235,254,255,0.92)" : dark ? "rgba(255,244,254,0.92)" : tone === "purple" ? "rgba(245,236,255,0.92)" : "#FFFFFF"
  const fg = colors.ink
  const content = (
    <>
      <View style={[s.methodIcon, { backgroundColor: "rgba(255,255,255,0.72)" }]}>
        <Text style={[s.methodIconText, { color: dark ? "#A5A299" : "#75736A" }]}>{icon}</Text>
      </View>
      <View style={{ flex: 1 }}>
        <Text style={[s.methodTitle, { color: fg, fontFamily: fonts.displayHeavy }]}>{title}</Text>
        <Text style={[s.methodSub, { color: "#75736A" }]}>{sub}</Text>
      </View>
      <View style={{ alignItems: "flex-end" }}>
